...
proc make_beam_many_energy {name nslice n de} {
    global charge e_initial match n_total
    set energy [expr $e_initial * $de]
    set ch {1.0}
    calc beam.dat $charge -3 3 $match(sigma_z) $nslice
    InjectorBeam $name -bunches 1 \
            -macroparticles [expr $n] \
            -particles $n_total \
            -energyspread [expr 0.01*$match(e_spread)*$energy] \
            -ecut 3.0 \
            -e0 $energy \
            -file beam.dat \
            -chargelist $ch \
    ...
    lappend l {1.0 0.0 0.0}
    SetRfGradientSingle $name 0 $l
    make_beam_particles $energy $match(e_spread) [expr $nslice*$n]
    BeamRead -file particles.in -beam $name
}
...
proc make_beam_train_main {name nb nslice n} {
    global charge e_initial match wk_scale
    set ch {1.0}
    calc beam.dat $charge -3 3 $match(sigma_z) $nslice
    set wl {}
